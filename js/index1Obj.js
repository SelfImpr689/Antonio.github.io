...
//	首页对象初始化
	...
	getGroupList:function(userID,shouldEmpty){
		//$('.mop-load-0').show();
		//indexObj.mopload();
		var url = '/survivor/rest/ControlService/getPicDatas/'+userID+'?pageNum='+index1Obj.pageNum;
		$.ajax({
			type:"get",
			url:url,
			async:true,
...
var loginObj = {
	...
	getUser: function(){
		$.ajax({
			type:"get",
			url:"/survivor/rest/UserService/getCurrentUser",
			async:true,
			dataType:'json',
			success: function(data){
			  if(data.message==-1){
	...
	logout:function(){
		$.ajax({
			type:"get",
			url:"/survivor/rest/UserService/logout",
			async:true,
			dataType:'json',
			success: function(data){
    ...
    submitForm:function(){
        $("#login-form-submit").on("click",function(){
            ...
            loginObj.submitOption = {
                type:"POST",
                url:"/survivor/rest/UserService/login",
                async:true,
                data:{
                	userName: userName,
